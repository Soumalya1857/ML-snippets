...
class NimAI():
    ...
    def update_q_value(self, state, action, old_q, reward, future_rewards):
        ...
        new_value_estimation = (current_reward + epsilon(max of all possible reward from that state))
    ...
    def best_future_reward(self, state):
        ...
        if not Nim.available_actions(state):
            return 0
            #accounts the condition of no available action
    ...
    def choose_action(self, state, epsilon=True):
        ...
        choose = [random_action,best_action]

        if epsilon == False:
            return best_action
        
        if epsilon == True:
            temp = random.choices(choose,weight)
            return temp[0]
